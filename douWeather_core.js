{
    ...
    class DouWeatherCard extends HTMLElement {
        ...
        constructor() {
            ...
            this.setAttribute("role", "weather-card");
        ...
        update() {
            ...
            fetch(`https://api.daidr.me/apis/weather?location=${location}`)
                ...
                .then(json => {
                    ...
                    if (type != "small") {
                        // 右侧天气数据
                        this.windLevel = content.querySelector('.data-wind').innerText = getWindLevel(weatherData.current.wind.speed.value);
                        let windDirectionData = this.windDirection = getWindDirection(weatherData.current.wind.direction.value);
        ...
        setPositionText(text) {
            this.shadowRoot.querySelector(".position-text")
                .textContent = text;
            if (text == "") {
